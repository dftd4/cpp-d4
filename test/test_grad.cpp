...
int test_numgrad(TMolecule &mol, const int charge, const dparam &par) {
  ...
  for (int i = 0; i < mol.NAtoms; i++) {
    for (int c = 0; c < 3; c++) {
      ...
      mol.xyz(i, c) += step;
      get_dispersion(mol, par, charge, cutoff, er, nullptr);

      mol.xyz(i, c) = mol.xyz(i, c) - 2*step;
      get_dispersion(mol, par, charge, cutoff, el, nullptr);
  ...
  info = get_dispersion(mol, par, charge, cutoff, energy, d4grad);
...
int test_bp86d4atm_water() {
  ...
  // assemble molecule
  TMolecule mol;
  info = get_molecule(water_n, water_atoms, water_coord, mol);
  if (!info == EXIT_SUCCESS) return info;
  charge = water_charge; 
...
int test_tpss0d4mbd_rost61m1() {
  ...
  // assemble molecule
  TMolecule mol;
  info = get_molecule(rost61_m1_n, rost61_m1_atoms, rost61_m1_coord, mol);
  if (!info == EXIT_SUCCESS) return info;
  charge = rost61_m1_charge; 
